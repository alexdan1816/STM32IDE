...
float s_require = 160;
float s_remain = 0;
//*********************Speed profile of move forward; đơn vị mm & s
float v_stop = 0;  // mm/s
float v_max = 356; // mm/s (giữ như bạn đang dùng)
float v_next = 0;  // mm/s (tham chiếu tuyến tính)

float a_up = 1200;	 // mm/s^2
float a_down = 1200; // mm/s^2

// ---- S-curve (jerk-limited) ----
float j_up = 8000;	 // mm/s^3  (tăng tốc “bén” hơn => tăng số này, rung thì giảm)
float j_down = 8000; // mm/s^3  (giảm tốc)
float a_cur = 0;	 // mm/s^2  (gia tốc đang dùng theo S-curve)
...
void Move_forward(Motor *_motorL, Motor *_motorR)
{
	if (cur_state == MOVE && !begin_flag)
	{
		...
		// Reset profile S-curve
		v_next = 0.0f; // mm/s
		a_cur = 0.0f;  // mm/s^2

		begin_flag = true;

		gyro_set_point = angle;
		gyro_pid_in = angle - gyro_set_point;
	...
	//	v_next = v_next / MMS;
	vR = (double)(v_next / MMS) + gyro_pid_out;
	vL = (double)(v_next / MMS) - gyro_pid_out;
	Motor_SetTarget(_motorR, (double)vR);
	Motor_SetTarget(_motorL, (double)vL);
}
...
void Move_Left(Motor *_motorL, Motor *_motorR)
{
	...
	else if (cur_state == TURN_LEFT && begin_flag)
	{
		...
		if (encoder_target - encoder_progress < TURN_TOLERANCE || angle - 89 >= 0)
		{
			_motorL->Pid_output = 0;
			_motorR->Pid_output = 0;
			//			Motor_SetPwm(_motorR);
...
void Move_Right(Motor *_motorL, Motor *_motorR)
{
	...
	else if (cur_state == TURN_RIGHT && begin_flag)
	{
		...
		if (encoder_target - encoder_progress < TURN_TOLERANCE || fabs(angle) - 89 > 0)
		{
			_motorL->Pid_output = 0;
			_motorR->Pid_output = 0;
			//			Motor_SetPwm(_motorR);
